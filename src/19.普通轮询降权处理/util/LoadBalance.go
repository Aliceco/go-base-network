...
func(this *LoadBalance) RoundRobinByWeight2() *HttpServer  {
	...
	sum:=0
	// 3:1:1
	fmt.Println("len(this.Servers)---",len(this.Servers))
	for i:=0;i<len(this.Servers);i++{
		...
		if this.CurIndex<sum {
			...
			if this.CurIndex==sum-1 && i!=len(this.Servers)-1{
				fmt.Println(sum,this.CurIndex,"up")
				this.CurIndex++
			} else {
				this.CurIndex=(this.CurIndex+1)%sum //这里是重要的一步
				fmt.Println(sum,this.CurIndex)
			}
			break
		}
	}
	fmt.Println(sum,this.CurIndex,"last")
	return server
}
...
func init()  {
	...
	LB.AddServer(NewHttpServer("http://localhost:9093", 2))
	...
	for {
		...
		case <- t.C:
			...
			for _,s:=range servers {
				fmt.Println(s.Host, s.Status, s.FailCount, s.SuccessCount, s.Weight, s.FailWeight)
			}
			fmt.Println("----------------")
		}
	}
